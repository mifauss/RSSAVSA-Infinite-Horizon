...
function xk_plus1 = bidirectional_flow_by_gravity_with_cso_tank1drain( xk, uk, wk, cfg, scn, amb )
...
% discharge rate to storm sewer from tank 2
q_storm_sewer_from_tank_2 = q_outlet_linearized(xk(2), 1, SSR, SSZ, amb.xmax(2)); 

% discharge rate to storm sewer from tank 1
q_storm_sewer_from_tank_1 = q_outlet_linearized(xk(1), 1, SSR, SSZ, amb.xmax(1));
...
f(1) = ( w1 - q_to_combined_sewer_1 - q_valve - q_storm_sewer_from_tank_1 ) / scn.surface_area(1);  % time-derivative of x1 [ft/s]
f(2) = ( w2 - q_to_combined_sewer_2 + q_valve - q_storm_sewer_from_tank_2 ) / scn.surface_area(2);  % time-derivative of x2 [ft/s]
